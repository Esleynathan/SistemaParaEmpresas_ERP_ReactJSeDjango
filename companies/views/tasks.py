...
class Tasks(Base):
    ...
    def post(self, request):
        ...
        if not title or len(title) > 125:
            raise APIException("O título é obrigatório e deve ter no máximo 125 caracteres")

        if due_date:
            try:
                due_date = datetime.datetime.strptime(due_date, "%d/%m/%Y %H:%M")
            except ValueError:
                raise APIException("A data de entrega deve estar no formato d/m/Y H:M", "date_invalid")       
...
class TaskDetail(Base):
    ...
    def put(self, request, task_id):
        ...
        if not serializer.is_valid():
            raise APIException("Não foi possível editar a tarefa:", serializer.errors)
